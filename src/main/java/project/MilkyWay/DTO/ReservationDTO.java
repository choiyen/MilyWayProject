...
public class ReservationDTO //고객의 예약을 관리하기 위한 DTO
{
    ...
    @Schema(description = "예약일정을 관리하기 위한 Id", example = "2024-10-11", required = true) //예약 일정이 있어야 들어가는 게 아니라, 예약 일정이 없다면 생성하고, 데이터 추가 작업 수행
    ...
    @Schema(description = "실 평수", example = "25평", required = true)
    String acreage;
    ...
    @Schema(description = "대략 적으로 방별로 몇개가 있는가?", example = "방3/화장실2/베란다2/부엌1", required = true)
    ...
    @Schema(description = "예약 고객의 이름", example = "홍길성", required = true)
    private String name; // 고객의 이름

    @NotBlank(message = "phone cannot be empty")
    @Size(min = 5, max= 12, message = "phone는 최소 다섯자리 이상 입력해야 함.")
    @Pattern(regexp = "^[0-9]+$", message = "숫자만 입력 가능합니다.")
    @Schema(description = "예약 고객의 전화번호", example = "010-1111-1111", required = true)
