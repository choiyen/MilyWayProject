...
function handleServerError(response) {
    ...
    if (resultType === "error" && message) {
        ...
        const errors = message
            .split(/[.\n]/) // 마침표나 줄바꿈 기준으로 나눔
            .map((msg) => msg.trim())
            .filter((msg) => msg.length > 0); // 빈 항목 제거
        if (errors.length > 0) {
...
export const InqurieInsert = async (Selector, dispatch) => {
    const timezoneOffset = new Date().getTimezoneOffset() * 60 * 1000;
    const curDate = new Date(Date.now() - timezoneOffset);
    dispatch(setIqurieData({ ...Selector, SubmissionDate: curDate.toISOString() }));
    await POST({
        ...
        .catch((err) => {
        if (err.response) {
            handleServerError(err.response.data);
        }
        else {
            Swal.fire({
                icon: "error",
