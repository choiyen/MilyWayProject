import bathsrooms from "@/Components/Common/assets/bathsrooms.png";
import there_image from "@/Components/Common/assets/cleaning_blog_assets/there_image.jpg";
import bathsroom from "@/Components/Common/assets/bathroom.jpg";
import cleaningroom from "@/Components/Common/assets/cleaningroom.jpg";
import divcleaning from "@/Components/Common/assets/divcleaning.jpg";
import RoomClean from "@/Components/Common/assets/RoomClean.jpg";
import two_image from "@/Components/Common/assets/cleaning_blog_assets/two_image.jpg";
import building from "@/Components/Common/assets/cleaning_blog_assets/building.png";
import post_construction from "@/Components/Common/assets/cleaning_blog_assets/post-construction.png";
import balcony from "@/Components/Common/assets/movein/KakaoTalk_20250517_205032360_19.jpg";
import room from "@/Components/Common/assets/movein/KakaoTalk_20250517_205032360_04.jpg";
import sink from "@/Components/Common/assets/movein/KakaoTalk_20250517_204915707_05.jpg";
import homroom_room from "@/Components/Common/assets/movein/homroom_room.jpg";
import homroom_sink from "@/Components/Common/assets/movein/homroom_sink.jpg";
import newhome_bathrooms from "@/Components/Common/assets/newhome/newhome_bathrooms.jpg";
import newhome_room from "@/Components/Common/assets/newhome/newhome_room.jpg";
import newhome_sink from "@/Components/Common/assets/newhome/newhome_sink.jpg";
import steps from "@/Components/Common/assets/steps/steps.png";
import stepsnew from "@/Components/Common/assets/steps/stepsnew.png";
import stepsbalcony from "@/Components/Common/assets/steps/stepsbalcony.png";
import newhouse from "@/Components/Common/assets/steps/newhouse.jpg";
import stepclean from "@/Components/Common/assets/steps/stepclean.png";
import newout from "@/Components/Common/assets/steps/newout.jpg";
import divcleanings from "@/Components/Common/assets/buliding/divcleanings.png";
import foodclean from "@/Components/Common/assets/buliding/foodclean.png";
import hometown from "@/Components/Common/assets/buliding/hometown.png";

export interface TabService {
  ImageComment: string;
  name: string;
  ServiceList: string[];
}

export interface Introduction {
  Service: string;
  Introduction: string;
  defaultImg: string;
  ServiceList: TabService[];
}
export const IntroductionService: Introduction[] = [
  {
    Service: "전체보기",
    Introduction:
      "은하수 홈케어는 고객의 생활 공간을 보다 쾌적하고 위생적으로 유지하기 위해 다양한 전문 청소 서비스를 제공합니다. 이사청소, 입주청소, 주거청소 등 주거공간을 위한 서비스부터 계단, 준공현장, 공공시설까지 폭넓게 대응합니다. 각각의 서비스는 환경과 용도에 맞는 전문 장비와 인력을 바탕으로 제공되며, 눈에 보이는 오염뿐 아니라 숨어있는 위생 사각지대까지 책임집니다. 맞춤형 상담과 섬세한 작업을 통해 고객의 삶의 질을 높이는 것이 저희의 목표입니다.",
    defaultImg: bathsrooms,
    ServiceList: [
      {
        ImageComment: cleaningroom,
        name: "이사청소",
        ServiceList: [
          "1. 욕실·주방 묵은 때 제거",
          "2. 벽면·바닥 오염물 세척",
          "3. 창틀·방충망 먼지 제거",
          "4. 문틀·몰딩 곰팡이 제거",
          "5. 가구 자국·자취 흔적 정리",
          "6. 분리수거 및 잔여물 정리",
          "7. 쓰레기 배출 및 마무리 청소",
        ],
      },
      {
        ImageComment: divcleaning,
        name: "주거청소",
        ServiceList: [
          "1. 욕실 곰팡이 및 물때 제거",
          "2. 주방 기름때 클리닝",
          "3. 바닥 먼지 및 오염 제거",
          "4. 창문·샷시 먼지 정리",
          "5. 문틀·스위치 등 생활 오염 청소",
          "6. 침실·거실 정리 청소",
          "7. 쓰레기 분리 및 간단한 정돈",
        ],
      },
      {
        ImageComment: two_image,
        name: "입주청소",
        ServiceList: [
          "1. 건축 자재 먼지 제거",
          "2. 새집 냄새 제거 및 환기",
          "3. 창호·샷시·창틀 클리닝",
          "4. 붙박이장 내부 청소",
          "5. 몰딩·조명 스위치 닦기",
          "6. 욕실·주방 입주 전 클리닝",
          "7. 바닥 마감 클리닝 및 광택",
        ],
      },
      {
        ImageComment: there_image,
        name: "계단청소",
        ServiceList: [
          "1. 계단 먼지 및 쓰레기 제거",
          "2. 벽면 손자국·오염 제거",
          "3. 난간·손잡이 소독",
          "4. 입구·출입문 바닥 청소",
          "5. 계단 코너 먼지 제거",
          "6. 스티커·테이프 흔적 제거",
          "7. 구석 청소 및 물청소",
        ],
      },
      {
        ImageComment: building,
        name: "건물청소",
        ServiceList: [
          "1. 외부 유리창 물청소",
          "2. 공용부 바닥·복도 청소",
          "3. 화장실 정기 클리닝",
          "4. 엘리베이터 내부 소독",
          "5. 계단 및 창고 정리 청소",
          "6. 건물 출입구 먼지 제거",
          "7. 전등·스위치 외관 정리",
        ],
      },
      {
        ImageComment: RoomClean,
        name: "새집증후군",
        ServiceList: [
          "1. 새집 냄새 제거 (피톤치드 등)",
          "2. 벽면·바닥 유해물질 제거",
          "3. 포름알데히드 저감 작업",
          "4. 환기 후 필터 교체 안내",
          "5. 천장·창틀 미세먼지 청소",
          "6. 고온 스팀 살균 소독",
          "7. 전체 공간 탈취 및 공기 정화",
        ],
      },
      {
        ImageComment: post_construction,
        name: "준공청소",
        ServiceList: [
          "1. 공사 잔해 및 먼지 제거",
          "2. 유리창·샷시 마감 클리닝",
          "3. 바닥 코팅 전 청소",
          "4. 붙박이장·몰딩 먼지 제거",
          "5. 벽지 접착제 잔여물 제거",
          "6. 화장실 및 주방 초벌청소",
          "7. 전체 공간 마무리 클리닝",
        ],
      },
      {
        ImageComment: bathsroom,
        name: "부분청소",
        ServiceList: [
          "1. 욕실 집중 클리닝",
          "2. 주방 싱크대·가스레인지 청소",
          "3. 방 1~2개 선택 청소",
          "4. 베란다·발코니 곰팡이 제거",
          "5. 창문·창틀 구역 청소",
          "6. 현관/신발장 부분 정리",
          "7. 고객 요청 부위 맞춤 청소",
        ],
      },
    ],
  },
  {
    Service: "이사청소",
    Introduction:
      "이사청소는 다른 사람이 사용했던 공간을 처음처럼 새롭게 만드는 서비스입니다. 집을 비운 직후나 입주 직전에 진행되며, 바닥, 창틀, 싱크대, 욕실, 벽면 등 오염이 집중된 부분을 꼼꼼하게 청소합니다. 특히 장판 밑 먼지, 찌든 때, 곰팡이 흔적 등 일반 청소로는 어려운 오염까지 제거합니다. 이사는 새 출발의 시작이기에, 깨끗한 환경에서 시작할 수 있도록 도와드립니다. 쾌적한 새집의 기분을 느끼고 싶다면 꼭 필요한 필수 서비스입니다.",
    defaultImg: cleaningroom,
    ServiceList: [
      {
        ImageComment: balcony,
        name: "현관, 베란다",
        ServiceList: [
          "1. 현관 바닥 묵은 때 및 먼지 제거",
          "2. 신발장 외관 및 내부 청소",
          "3. 베란다 곰팡이·물때 제거",
          "4. 난간 및 유리창 닦기",
          "5. 배수구 주변 정리 및 냄새 제거",
          "6. 창문틀 먼지 및 이물질 제거",
          "7. 모서리 구석 집중 청소",
        ],
      },
      {
        ImageComment: room,
        name: "거실, 방",
        ServiceList: [
          "1. 바닥 청소 및 얼룩 제거",
          "2. 장판 밑 오염물 제거 (이사 전용)",
          "3. 벽면 및 몰딩 먼지 제거",
          "4. 붙박이장 내부 클리닝",
          "5. 콘센트, 스위치 외관 닦기",
          "6. 창틀·방충망 청소",
          "7. 조명·천장 부위 거미줄 제거",
        ],
      },
      {
        ImageComment: bathsroom,
        name: "욕실, 화장실",
        ServiceList: [
          "1. 변기, 세면대, 욕조 찌든 때 제거",
          "2. 곰팡이 및 물때 제거",
          "3. 타일 사이 곰팡이 제거 및 세척",
          "4. 샤워부스 유리 물때 닦기",
          "5. 환풍기 덮개 먼지 제거",
          "6. 배수구 주변 청소 및 탈취",
          "7. 거울, 유리문 얼룩 제거",
        ],
      },
      {
        ImageComment: sink,
        name: "주방, 싱크대",
        ServiceList: [
          "1. 싱크대 상하부장 내부 청소",
          "2. 가스레인지 및 후드 기름때 제거",
          "3. 조리대 및 타일 벽면 클리닝",
          "4. 싱크대 배수구 세척 및 탈취",
          "5. 수납장 손잡이·몰딩 닦기",
          "6. 조리기구 설치 공간 정리",
          "7. 냉장고 외관 및 하부 먼지 제거",
        ],
      },
    ],
  },
  {
    Service: "주거청소",
    Introduction:
      "주거청소는 현재 거주 중인 공간을 주기적으로 정리하고 위생적으로 관리하는 서비스입니다. 집 안의 각종 생활 먼지, 주방의 기름때, 욕실의 곰팡이, 창틀과 방충망의 미세먼지 등은 시간이 지날수록 건강에 영향을 미칠 수 있습니다. 전문 청소도구와 친환경 세제를 이용해 공간의 구석구석을 관리하고, 고객의 생활 리듬에 맞춰 청소 일정을 조정할 수 있어 바쁜 현대인에게 최적화된 서비스입니다. 더 나은 주거환경과 가족의 건강을 지키기 위한 현명한 선택입니다.",
    defaultImg: two_image,
    ServiceList: [
      {
        ImageComment: balcony,
        name: "현관, 베란다",
        ServiceList: [
          "1. 현관 바닥 먼지 제거 및 물걸레질",
          "2. 신발장 외부 정리 및 문 손잡이 소독",
          "3. 베란다 바닥 및 배수구 청소",
          "4. 세탁기 외부 및 연결 호스 먼지 제거",
          "5. 창문 및 난간 닦기",
          "6. 빨래 건조 공간 위생 관리",
          "7. 해충 유입 경로 점검",
        ],
      },
      {
        ImageComment: homroom_room,
        name: "거실, 방",
        ServiceList: [
          "1. 바닥 먼지 제거 및 물걸레 청소",
          "2. 가구 주변·아래 먼지 제거",
          "3. 침구 정리 및 먼지 털기",
          "4. 책상·서랍장 상단 정리",
          "5. 조명 및 스위치 닦기",
          "6. 창틀·방충망 정기 세척",
          "7. 공기청정기 및 가전 외관 정리",
        ],
      },
      {
        ImageComment: bathsroom,
        name: "욕실, 화장실",
        ServiceList: [
          "1. 세면대 및 변기 청소 및 소독",
          "2. 거울 및 유리면 얼룩 제거",
          "3. 샤워부스 및 벽면 물때 제거",
          "4. 타일 사이 곰팡이 초기 제거",
          "5. 욕실 매트 세척 및 정리",
          "6. 환풍구 먼지 제거",
          "7. 배수구 청소 및 탈취제 처리",
        ],
      },
      {
        ImageComment: homroom_sink,
        name: "주방, 싱크대",
        ServiceList: [
          "1. 싱크대 내부 정리 및 물때 제거",
          "2. 가스레인지 및 인덕션 기름때 청소",
          "3. 조리대 소독 및 타일벽 닦기",
          "4. 음식물 쓰레기통 및 주변 정리",
          "5. 수납장 외부 먼지 닦기",
          "6. 전자레인지 외관 정리",
          "7. 냉장고 외부 먼지 및 손잡이 소독",
        ],
      },
    ],
  },
  {
    Service: "입주청소",
    Introduction:
      "입주청소는 새로 지어진 아파트나 리모델링을 마친 공간에서 입주 전 실시하는 전문 청소입니다. 시공 후 남은 분진, 유리나 타일의 접착제 자국, 건축자재의 흔적 등을 말끔히 정리해 완전한 새집으로 탈바꿈시킵니다. 일반적인 청소로는 제거하기 힘든 오염을 고성능 장비와 전문 인력을 통해 처리하며, 특히 알레르기나 피부 질환이 걱정되는 어린 자녀가 있는 가정에 필수적인 서비스입니다. 새 출발의 첫 공간을 건강하고 깨끗하게 준비해드립니다.",
    defaultImg: divcleaning,
    ServiceList: [
      {
        ImageComment: balcony,
        name: "현관, 베란다",
        ServiceList: [
          "1. 현관 바닥 시공 잔여물 제거",
          "2. 신발장 내부 및 외부 분진 제거",
          "3. 베란다 타일 먼지 및 시멘트 가루 제거",
          "4. 창문 실리콘 자국 및 유리 클리닝",
          "5. 배수구 내부 오염 제거 및 탈취",
          "6. 실리콘·접착제 이물질 제거",
          "7. 난간 및 외부 유리 먼지 정리",
        ],
      },
      {
        ImageComment: newhome_room,
        name: "거실, 방",
        ServiceList: [
          "1. 바닥 분진 및 미세 시멘트 가루 제거",
          "2. 몰딩 주변 잔여 실리콘 제거",
          "3. 조명 커버 및 스위치 먼지 제거",
          "4. 벽면 타일·패널 얼룩 닦기",
          "5. 창틀 안쪽 건축 먼지 제거",
          "6. 콘센트 및 통신단자 오염 정리",
          "7. 바닥 마감재 보호필름 제거 및 청소",
        ],
      },
      {
        ImageComment: newhome_bathrooms,
        name: "욕실, 화장실",
        ServiceList: [
          "1. 타일 벽면·바닥 시공 잔여물 제거",
          "2. 거울·유리면 실리콘·먼지 제거",
          "3. 욕조 및 세면대 실리콘 얼룩 정리",
          "4. 배수구 이물질 제거 및 소독",
          "5. 타일 줄눈 사이 먼지 제거",
          "6. 환풍기 및 천장 오염 제거",
          "7. 문턱 및 문틀 하단 세부 클리닝",
        ],
      },
      {
        ImageComment: newhome_sink,
        name: "주방, 싱크대",
        ServiceList: [
          "1. 싱크대 내부 공사 분진 제거",
          "2. 조리대 상판 및 타일 벽면 청소",
          "3. 후드 외관 기름막 제거",
          "4. 상·하부장 내부 먼지 및 얼룩 제거",
          "5. 수도꼭지·배관 얼룩 제거 및 광택 작업",
          "6. 가스렌지·인덕션 주변 보호비닐 제거",
          "7. 수납장 경첩 주변 이물질 제거",
        ],
      },
    ],
  },
  {
    Service: "계단청소",
    Introduction:
      "계단청소는 아파트, 오피스텔, 상가 등 다중 이용시설의 공용 계단을 대상으로 하는 전문 서비스입니다. 계단은 많은 사람이 이용하면서도 관리가 소홀해지기 쉬운 공간입니다. 먼지, 낙서, 물자국, 껌, 기름때 등 다양한 오염을 제거하며, 바닥 코팅 및 난간 소독 등도 포함됩니다. 미끄럼 사고 예방과 위생적인 환경 유지는 입주민과 방문자의 만족도와 직결되기 때문에 정기적인 관리가 필요합니다. 은하수 홈케어는 시설의 품격을 높이는 계단 관리를 제공합니다.",
    defaultImg: there_image,
    ServiceList: [
      {
        ImageComment: steps,
        name: "계단 바닥",
        ServiceList: [
          "1. 바닥 먼지 및 이물질 제거 (빗자루·진공청소기 활용)",
          "2. 껌, 낙서, 얼룩 제거 전문 클리닝",
          "3. 미끄럼 방지 코팅 및 재도포",
          "4. 바닥 타일·콘크리트 표면 세척",
          "5. 스테인리스 또는 금속 마감 부분 광택 및 세척",
          "6. 배수구 청소 및 막힘 제거",
          "7. 안전사고 예방용 경고표지 부착",
        ],
      },
      {
        ImageComment: stepsnew,
        name: "난간 및 벽면",
        ServiceList: [
          "1. 난간 손잡이 소독 및 세척",
          "2. 난간 도장면 먼지 및 오염 제거",
          "3. 벽면 낙서 및 오염물 제거",
          "4. 벽면 먼지 제거 및 얼룩 닦기",
          "5. 조명 커버 및 스위치 청소",
          "6. 비상벨, 손잡이 등 접촉 빈도 높은 부분 소독",
          "7. 안전시설 점검 및 경고 표지 관리",
        ],
      },
      {
        ImageComment: stepsbalcony,
        name: "입구 및 휴게 공간",
        ServiceList: [
          "1. 입구 바닥 및 매트 청소",
          "2. 쓰레기통 정리 및 소독",
          "3. 우산꽂이 및 벤치 등 부속 시설 청소",
          "4. 안내판 및 벽면 먼지 제거",
          "5. CCTV 카메라 외부 청소",
          "6. 공기 순환구 점검 및 청소",
          "7. 고객 요청 특수 구역 맞춤 청소",
        ],
      },
    ],
  },
  {
    Service: "새집증후군",
    Introduction:
      "새집증후군은 신축 건물에서 발생하는 유해물질로 인한 불편함을 줄여주는 서비스입니다. 은하수 홈케어는 친환경 약제를 사용해 실내 공기질을 개선하고, 건강한 환경을 만들기 위한 기본 케어를 제공합니다. 복잡한 절차 없이 신속하게 유해물질을 줄이고 쾌적한 공간을 조성합니다.",
    defaultImg: RoomClean,
    ServiceList: [
      {
        ImageComment: newhouse,
        name: "새집증후군 케어",
        ServiceList: [
          "1. 친환경 탈취제 및 중화제 약제 분사",
          "2. 주요 공간 및 환기구 집중 케어",
          "3. 시공 후 남은 잔여 냄새 감소 작업",
          "4. 간단한 환기 및 공기 순환 지원 안내",
          "5. 필요 시 후속 약제 재분사 서비스 안내",
        ],
      },
    ],
  },
  {
    Service: "준공청소",
    Introduction:
      "준공청소는 상가, 오피스, 주택 등 공사 후 완공된 현장을 깨끗하게 정리하는 작업입니다. 시멘트 가루, 실리콘 자국, 먼지와 페인트 찌꺼기 등을 제거해 입주 전 최상의 상태로 마무리합니다. 일반적인 생활 청소보다 더 전문적인 장비와 경험이 필요하며, 미관은 물론 위생과 안전까지 고려한 작업이 이루어집니다. 건물의 첫인상을 결정짓는 중요한 과정이므로, 디테일한 마감 청소가 필수입니다. 은하수 홈케어는 현장 맞춤형 준공청소를 제공합니다.",
    defaultImg: post_construction,
    ServiceList: [
      {
        ImageComment: steps,
        name: "바닥 및 표면 청소",
        ServiceList: [
          "1. 시멘트 가루 및 건축 먼지 제거",
          "2. 페인트 얼룩 및 접착제 잔여물 제거",
          "3. 타일 및 마루 표면 광택 및 클리닝",
          "4. 바닥 틈새 및 모서리 청소",
          "5. 난간 및 손잡이 세척",
        ],
      },
      {
        ImageComment: stepclean,
        name: "유리 및 창문 청소",
        ServiceList: [
          "1. 창문 유리 내부·외부 클리닝",
          "2. 창틀 및 창문 틈 먼지 제거",
          "3. 알루미늄 및 샷시 부분 세척",
          "4. 유리 얼룩 및 물때 제거",
        ],
      },
      {
        ImageComment: bathsrooms,
        name: "주방 및 욕실 마감 청소",
        ServiceList: [
          "1. 싱크대, 가스레인지 잔여물 제거",
          "2. 욕실 타일 및 배수구 청소",
          "3. 욕실 유리문 및 거울 닦기",
          "4. 실리콘 자국 및 곰팡이 제거",
        ],
      },
      {
        ImageComment: newout,
        name: "최종 점검 및 정리",
        ServiceList: [
          "1. 쓰레기 및 폐기물 처리",
          "2. 환기 및 냄새 제거 작업",
          "3. 고객 맞춤 요청 사항 반영 청소",
          "4. 안전 점검 및 마무리 정리",
        ],
      },
    ],
  },
  {
    Service: "부분청소",
    Introduction:
      "부분청소는 집 전체가 아닌 특정 공간이나 항목에 집중하여 진행하는 선택형 서비스입니다. 예를 들어 욕실, 화장실, 주방, 창문 청소 등 고객의 요청에 따라 맞춤형으로 진행됩니다. 전문 장비를 이용해 일반 청소로는 어려운 찌든 때나 곰팡이 등을 제거하며, 비용 부담을 줄이면서도 효율적인 결과를 기대할 수 있습니다. 바쁜 일정, 임박한 방문 일정 등 다양한 상황에서 활용도가 높은 유연한 서비스입니다.",
    defaultImg: bathsroom,
    ServiceList: [
      {
        ImageComment: newhome_bathrooms,
        name: "욕실 청소",
        ServiceList: [
          "1. 타일 및 벽면 곰팡이 제거",
          "2. 욕조 및 샤워부스 청소",
          "3. 욕실 유리문 및 거울 닦기",
        ],
      },
      {
        ImageComment: bathsrooms,
        name: "화장실 청소",
        ServiceList: [
          "1. 변기 및 세면대 클리닝",
          "2. 배수구 청소 및 냄새 제거",
          "3. 바닥 및 벽면 청소",
        ],
      },
      {
        ImageComment: sink,
        name: "주방 청소",
        ServiceList: [
          "1. 싱크대 및 가스레인지 클리닝",
          "2. 조리대 및 찬장 외부 닦기",
          "3. 냉장고 내부 정리 및 청소",
          "4. 전자레인지 및 오븐 외부 청소",
        ],
      },
      {
        ImageComment: stepclean,
        name: "창문 및 발코니 청소",
        ServiceList: [
          "1. 창문 유리 내·외부 닦기",
          "2. 창틀 및 방충망 먼지 제거",
          "3. 발코니 바닥 및 난간 청소",
        ],
      },
    ],
  },
  {
    Service: "건물청소",
    Introduction:
      "건물청소는 상업용 건물, 오피스, 공공기관 등 다양한 규모의 건물을 대상으로 하는 전문적인 청소 서비스입니다. 공용 공간, 복도, 로비, 화장실 등 위생 관리가 중요한 구역을 집중적으로 관리하며, 쾌적하고 청결한 환경을 유지합니다. 정기적인 유지관리 청소부터 집중적인 대청소까지, 고객의 운영 환경에 맞춰 맞춤형 서비스를 제공합니다. 은하수 홈케어는 건물의 가치를 높이고, 방문객과 입주자의 만족도를 향상시키기 위한 고품질의 클리닝 솔루션을 제공합니다.",
    defaultImg: building,
    ServiceList: [
      {
        ImageComment: foodclean,
        name: "음식점 청소",
        ServiceList: [
          "1. 주방 후드 및 배기구 클리닝",
          "2. 조리대 및 조리기구 표면 청소",
          "3. 바닥 및 테이블 위생 관리",
          "4. 화장실 청결 유지 및 소독",
          "5. 쓰레기 분리수거 및 처리",
        ],
      },
      {
        ImageComment: divcleanings,
        name: "상가 청소",
        ServiceList: [
          "1. 매장 내 바닥 및 벽면 청소",
          "2. 출입구 및 진열대 관리",
          "3. 공용 복도 및 계단 청소",
          "4. 화장실 및 휴게 공간 위생 관리",
          "5. 쓰레기 수거 및 분리수거",
        ],
      },
      {
        ImageComment: hometown,
        name: "사무실 청소",
        ServiceList: [
          "1. 책상 및 사무기기 외부 닦기",
          "2. 바닥 및 카펫 청소",
          "3. 회의실 및 공용 공간 위생 관리",
          "4. 화장실 청결 및 소독",
          "5. 쓰레기 수거 및 분리수거",
        ],
      },
    ],
  },
];
