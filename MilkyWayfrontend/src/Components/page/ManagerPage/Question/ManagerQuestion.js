...
export const ManagerQuestion = () => {
    const QuestionDatas = useSelector((state) => state.Question.value);
    ...
    useEffect(() => {
        PostQuestionALL().then((res) => {
            if (res.resultType === "findnot") {
                toast.error("현재 데이터베이스에 Q&A 정보가 없습니다. 새로운 정보를 추가해주세요.", {
                    position: "top-center",
                    autoClose: 5000,
            ...
            PostQuestionALL().then(async (res) => {
                if (res.resultType === "findnot") {
                    ...
                    const newOrUpdatedList = QuestionDatas.filter((q) => {
                        const match = res.data.find((e) => e.id === q.id);
                        // 신규 데이터 (id 없음) → 추가
                        if (!q.id)
                            return true;
    ...
    return (_jsx("div", { children: _jsxs(MainBox, { children: [_jsx(Fontname, { children: " Q&A \uAD00\uB9AC" }), _jsxs(Wapper, { children: [[...Array(count)].map((_, i) => (_jsxs("div", { ref: i === count - 1 ? lastItemRef : null, style: { gap: "50px", width: "100%" }, children: [_jsx(InputTextBox, { name: "Q&A 질문 (" + (i + 1) + ")", place: "예상 질문을 입력해주세요.", Value: Question, setValue: SetQuestion, index: i }), _jsx(TextAreaBox, { name: "Q&A 내용 (" + (i + 1) + ")", place: "질문에 대한 내용을 입력해주세요", Value: Comment, setValue: SetComment, index: i })] }, i))), _jsx(ImgTag, { src: plus, onClick: cleanCount })] }), _jsx(LastButton, { onClick: handleQuestion, children: " \uC7AC\uC5C5\uB85C\uB4DC" })] }) }));
};
