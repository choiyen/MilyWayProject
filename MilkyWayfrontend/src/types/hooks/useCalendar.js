...
const useCalendar = () => {
     ...
     */
    const getStartDayIndex = (date) => {
        const day = new Date(date.getFullYear(), date.getMonth(), 1).getDay();
        return (day + 6) % 7; // 월요일 기준으로 정렬
    };
    const startDayIndex = getStartDayIndex(currentDate);
    // 💡 이전 달 자리 채우기 (빈 칸)
    const prevDayList = Array.from({ length: startDayIndex }).map(() => DEFAULT_TRASH_VALUE);
    // 💡 이번 달 실제 날짜 리스트
    ...
    const weekCalendarList = currentCalendarList.reduce((acc, cur, index) => {
