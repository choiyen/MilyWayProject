...
const ResponseInterceptor = {
    ouFulfilled: (response) => {
        const contentType = response.headers["content-type"]?.toString();
        const isJson = contentType?.includes("application/json");
        if (isJson) {
            try {
                if (response.data === "목표 생성 성공")
                    return response;
        ...
        if (response.status >= 400) {
            ...
            const messageInConfig = response.data.message || response.data?.error?.message;
            const message = typeof messageInConfig === "string"
                ? messageInConfig
    ...
    onRejected: async (error) => {
        ...
        if (!response) {
            toast.error("서버와의 연결에 문제가 발생했습니다. 인터넷 연결을 확인해주세요.", {
                position: "top-center",
                autoClose: 5000,
...
axiosInstance.interceptors.response.use(ResponseInterceptor.ouFulfilled, ResponseInterceptor.onRejected);
const curringMethod = (method) => async (requestConfig) => {
